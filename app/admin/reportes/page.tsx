...
export default async function AdminReportesPage({ searchParams }: PageProps) {
      ...
      node: (
        <section className="overflow-hidden crm-card shadow-sm">
          <div className="flex items-center justify-between border-b px-5 py-4">
            ...
            <span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-bold text-white">{trend.length} días</span>
          </div>
      ...
      node: (
        <section className="grid gap-6 xl:grid-cols-2">
          <article className="overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md shadow-sm">
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div>
                <h3 className="font-black crm-text-primary">Rentabilidad por servicio</h3>
                <p className="report-truncate text-xs text-slate-400">Ingreso, costo y margen por servicio.</p>
              </div>
              <span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-bold text-white">{r.byService.length} servicios</span>
            </div>
            <div className="overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-transparent text-[10px] uppercase tracking-wide text-cyan-100">
                  <tr>
                    <th className="px-2 py-2">Servicio</th>
                    <th className="px-2 py-2">Categ.</th>
                    ...
                    <th className="px-2 py-2 text-right">Comis.</th>
                ...
                <tbody>
                  {r.byService.map((s, i) => (
                    <tr className={`border-t transition hover:bg-slate-800/40 ${i % 2 === 0 ? "" : "bg-slate-800/20"}`} key={`${s.servicio}-${s.categoria}`}>
                      <td className="max-w-[140px] px-2 py-2 font-semibold"><span className="report-truncate block">{s.servicio}</span></td>
                      <td className="max-w-[100px] px-2 py-2 capitalize text-slate-500"><span className="report-truncate block">{s.categoria.replace("_", " ")}</span></td>
                      <td className="px-2 py-2 text-right tabular-nums">{s.turnos}</td>
                      <td className="px-2 py-2 text-right tabular-nums font-semibold">{fmtMoney(s.ingresos)}</td>
                      <td className="px-2 py-2 text-right tabular-nums">{fmtMoney(s.costoInsumo)}</td>
                      <td className="px-2 py-2 text-right tabular-nums">{fmtMoney(s.comision)}</td>
                      <td className="px-2 py-2 text-right tabular-nums font-black">{fmtMoney(s.utilidadNeta)}</td>
                      <td className="px-2 py-2 text-right">
                        <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${s.rentabilidadNeta >= 0.4 ? "bg-emerald-100 text-emerald-700" : s.rentabilidadNeta >= 0.2 ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"}`}>
                          {pct(s.rentabilidadNeta)}
                        </span>
                      </td>
                    </tr>
                  ))}
                  {r.byService.length === 0 && <tr><td className="px-2 py-8 text-center text-slate-400" colSpan={8}>Sin turnos en el periodo.</td></tr>}
                </tbody>
              </table>
            </div>
          </article>
          <article className="overflow-hidden rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md shadow-sm">
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div>
                <h3 className="font-black crm-text-primary">Producción por empleado</h3>
                <p className="report-truncate text-xs text-slate-400">Producción, costo, comisión y utilidad para el negocio.</p>
              </div>
              <span className="rounded-full bg-white/15 px-2.5 py-1 text-[10px] font-bold text-white">{r.byEmployee.length} empleados</span>
            </div>
            <div className="overflow-hidden">
              <table className="w-full text-left text-xs">
                <thead className="bg-transparent text-[10px] uppercase tracking-wide text-cyan-100">
                  <tr>
                    ...
                    <th className="px-2 py-2">Espec.</th>
                    <th className="px-2 py-2 text-right">Turnos</th>
                ...
                <tbody>
                  {r.byEmployee.map((e, i) => (
                    <tr className={`border-t transition hover:bg-slate-800/40 ${i % 2 === 0 ? "" : "bg-slate-800/20"}`} key={`${e.empleado}-${e.especialidad}`}>
                      <td className="max-w-[140px] px-2 py-2 font-semibold"><span className="report-truncate block">{e.empleado}</span></td>
                      <td className="max-w-[100px] px-2 py-2 capitalize text-slate-500"><span className="report-truncate block">{e.especialidad.replace("_", " ")}</span></td>
                      <td className="px-2 py-2 text-right tabular-nums">{e.turnos}</td>
                      <td className="px-2 py-2 text-right tabular-nums font-semibold">{fmtMoney(e.ingresos + e.propinas)}</td>
                      <td className="px-2 py-2 text-right tabular-nums">{fmtMoney(e.costoInsumo)}</td>
                      <td className="px-2 py-2 text-right tabular-nums font-black text-violet-700">{fmtMoney(e.comision)}</td>
                      <td className="px-2 py-2 text-right tabular-nums font-black text-emerald-700">{fmtMoney(e.utilidadNegocio)}</td>
                    </tr>
                  ))}
                  {r.byEmployee.length === 0 && <tr><td className="px-2 py-8 text-center text-slate-400" colSpan={7}>Sin producción en el periodo.</td></tr>}
                </tbody>
              </table>
            </div>
          </article>
  ...
  return (
    <div className="space-y-6 print:space-y-4">
        ...
        <div className="relative">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">Rentabilidad operativa</h2>
            ...
            <div className="grid grid-cols-3 gap-2 no-print">
              {[{ label: "CSV servicios", tipo: "servicios" }, { label: "CSV empleados", tipo: "empleados" }, { label: "CSV pagos", tipo: "pagos" }].map(({ label, tipo }) => (
                <a key={tipo} className="flex items-center justify-center gap-1.5 rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 text-xs font-black text-white hover:bg-white/20 transition" href={`/admin/reportes/export-csv?from=${range.from}&to=${range.to}&tipo=${tipo}`} download>↓ {label}</a>
              ))}
      ...
      {/* ── Nota de pie: regla contable ─── */}
      <footer className="flex flex-wrap gap-x-6 gap-y-1 border-t border-white/8 pt-4 opacity-60">
        <p className="text-xs crm-text-muted">
          <span className="font-semibold">Regla de comisión:</span> {commissionBaseLabel[r.settings.comisionBase] || "Precio final"}
        </p>
        <p className="text-xs crm-text-muted">
          <span className="font-semibold">Propina en comisión:</span> {r.settings.propinaEnComision ? "Incluida" : "No incluida"}
        </p>
        <p className="text-xs crm-text-muted">
          <span className="font-semibold">Editable en:</span> Admin / Configuración
        </p>
      </footer>
