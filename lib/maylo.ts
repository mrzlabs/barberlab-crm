export interface MayloOpts {
  eyes?:  'open' | 'blink' | 'wink' | 'happy' | 'curious'
  mouth?: 'smile' | 'talk' | 'flat' | 'grin'
...
export function generateMaylo(opts?: MayloOpts): string {
  const o: Required<MayloOpts> = Object.assign(
    { eyes: 'open', mouth: 'smile', arms: 'wave', glow: true, panel: true, body: BRAND },
    opts ?? {},
  );
  const id = 'm' + (_uid++);
  ...
  const seg = (x: number, y: number, w: number, h: number, rot: number, px: number, py: number) =>
  ...
  /* ---------- legs ---------- */
  const leg = (cls: string, hx: number) => `
  ...
  /* ---------- arms ---------- */
  function armDown(cls: string, sx: number, dir: number) {
    const f = dir;
  ...
  function armUp(cls: string, sx: number) {
  ...
  let arms: string;
  if (o.arms === 'wave')
    arms = armDown('my-armL', 72, -1) + armUp('my-wave', 188);
  else if (o.arms === 'point')
    arms = armDown('my-armL', 72, -1) +
      `<g class="my-wave">${seg(186, 196, 48, 20, 4, 188, 206)}${ball(188, 196, 13)}${ball(232, 200, 11)}</g>`;
  else
    arms = armDown('my-armL', 72, -1) + armDown('my-wave', 188, 1);
  ...
  if (o.panel) {
    ...
    panel = `
      ...
      <circle cx="108" cy="220" r="5" fill="${P.cyan}" ${gl}/>
  ...
  /* ---------- visor + eyes ---------- */
  function lids(cx: number, cy: number, R: number, mode: string, side: string) {
    const lidF = `fill="url(#${id}visor)"`;
    const hood = `<path d="M${cx - R - 2} ${cy - R} h${2 * R + 4} v6 q-${R} ${R * .5} -${2 * R} 0 z" ${lidF}/>`;
    let shapes: string;
    if (mode === 'blink')
      shapes = `<rect x="${cx - R - 2}" y="${cy - R - 2}" width="${2 * R + 4}" height="${R + 4}" ${lidF}/>
        <rect x="${cx - R - 2}" y="${cy + R - 6}" width="${2 * R + 4}" height="${R + 8}" ${lidF}/>`;
    else if (mode === 'happy')
      shapes = `<path d="M${cx - R - 2} ${cy + R} h${2 * R + 4} v-${R + 4} q-${R} -${R * .9} -${2 * R} 0 z" ${lidF}/>`;
    else if (mode === 'curious')
      shapes = hood.replace('v6', 'v3');
    else
      shapes = hood;
    return `<g clip-path="url(#${id}e${side})">${shapes}</g>`;
  }

  function lens(cx: number, cy: number, R: number, side: string, mode: string) {
    return `
  ...
  const eyeMode   = (['blink', 'wink', 'happy', 'curious'] as const).includes(o.eyes as never) ? o.eyes : 'open';
  const leftMode  = o.eyes === 'wink' ? 'blink' : eyeMode;
  const rightMode = o.eyes === 'wink' ? 'open'  : eyeMode;
  const visor = `
